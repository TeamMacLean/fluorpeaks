...
recons_plot <- function(df){
  df1 <- dplyr::filter(df, line == "WT", reporter == "YFP")
  df2 <- dplyr::filter(df, line != "WT", reporter == "YFP")
  df3 <- dplyr::filter(df, line == "WT", reporter == "CFP")
  df4 <- dplyr::filter(df, line != "WT", reporter == "CFP")

  p1 <- ggplot2::ggplot(df1) + ggplot2::aes(minutes, reconstructed_intensity)  + ggplot2::geom_line(aes(colour = cell_number)) + ggplot2::facet_grid(bio_rep ~ tech_rep)
  p2 <- ggplot2::ggplot(df2) + ggplot2::aes(minutes, reconstructed_intensity)  + ggplot2::geom_line(aes(colour = cell_number)) + ggplot2::facet_grid(bio_rep ~ tech_rep)
  p3 <- ggplot2::ggplot(df3) + ggplot2::aes(minutes, reconstructed_intensity)  + ggplot2::geom_line(aes(colour = cell_number)) + ggplot2::facet_grid(bio_rep ~ tech_rep)
  p4 <- ggplot2::ggplot(df4) + ggplot2::aes(minutes, reconstructed_intensity)  + ggplot2::geom_line(aes(colour = cell_number)) + ggplot2::facet_grid(bio_rep ~ tech_rep)
